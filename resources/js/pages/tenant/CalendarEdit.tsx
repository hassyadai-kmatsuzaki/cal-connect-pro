...
const CalendarEdit: React.FC = () => {
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const [calendarResponse, usersResponse, formsResponse] = await Promise.all([
      ...
      // バックエンドのスネークケースをフロントエンドのキャメルケースに変換
      setFormData({
        name: calendar.name,
        type: calendar.type,
        ...
        displayInterval: calendar.display_interval,
        eventDuration: calendar.event_duration,
        daysInAdvance: calendar.days_in_advance,
        minHoursBeforeBooking: calendar.min_hours_before_booking,
        connectedUserIds: calendar.users?.map((u: any) => u.id) || [],
        inviteCalendars: calendar.invite_calendars || [],
        slackNotify: calendar.slack_notify || false,
        slackWebhook: calendar.slack_webhook || '',
        slackMessage: calendar.slack_message || '新しい予約が入りました。\n予約者: {name}\n日時: {datetime}',
        lineAutoReply: calendar.line_auto_reply || false,
        includeMeetUrl: calendar.include_meet_url ?? true,
        lineReplyMessage: calendar.line_reply_message || 'ご予約ありがとうございます。\n日時: {datetime}\n担当者: {staff}',
        lineRemind: calendar.line_remind || false,
        remindDaysBefore: calendar.remind_days_before || 0,
        remindHoursBefore: calendar.remind_hours_before || 24,
        lineRemindMessage: calendar.line_remind_message || '明日の予約のリマインドです。\n日時: {datetime}\n担当者: {staff}',
        hearingFormId: calendar.hearing_form_id || null,
      });
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const response = await axios.put(`/api/calendars/${id}`, payload);
      ...
      let errorMessage = 'カレンダーの更新に失敗しました';
                          ...
                          <Checkbox
                            ...
                            onChange={(e) => {
                              if (e.target.checked) {
                                ...
                                setFormData({
                                  ...formData,
                                  connectedUserIds: formData.connectedUserIds.filter((uid) => uid !== user.id),
                                });
                              }
                            }}
                          />
